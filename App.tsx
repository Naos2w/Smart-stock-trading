...
const DASHBOARD_TOOLTIPS = {
  zh: {
    ma: "MA5 > MA10 > MA20 多頭排列",
    vol: "今日成交量 > 5日均量",
    atr: "ATR 真實波幅 (停損依據)",
    rsi: "RSI 相對強弱 (>50多, >75熱)",
    ma240: "股價 > 年線 (240MA)",
    roe: "股東權益報酬率",
    yield: "現金殖利率",
    inst: "法人持股比例"
  ...
  en: {
    ma: "MA5 > MA10 > MA20 Bullish",
    vol: "Vol > Vol5 (Momentum)",
...
const StockTagBadge: React.FC<{ tag: StockTag, market: string }> = ({ tag, market }) => {
  ...
  return (
    <span 
      className={`px-2 py-0.5 rounded-md text-[10px] font-medium tracking-tight ${className}`}
...
const SegmentedControl = ({ options, value, onChange }: { options: {value: string, label: string}[], value: string, onChange: (v: any) => void }) => {
    return (
        <div className="bg-gray-200/50 dark:bg-gray-800/50 p-1 rounded-xl flex relative backdrop-blur-md">
            {options.map((opt) => (
                <button
                    key={opt.value}
                    onClick={() => onChange(opt.value)}
                    className={`flex-1 py-1.5 px-3 text-xs font-semibold rounded-[9px] transition-all duration-300 ease-out relative z-10 whitespace-nowrap ${
                        value === opt.value 
                        ? 'bg-white dark:bg-gray-600 text-black dark:text-white shadow-sm scale-100' 
                        : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 opacity-80 hover:opacity-100'
                    }`}
                >
                    {opt.label}
                </button>
...
export default function App() {
  ...
  const getCardStyle = () => {
      return 'bg-white dark:bg-dark-surface border border-transparent shadow-sm hover:shadow-md transition-all duration-300 active:scale-[0.99]';
  };
      ...
      return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className={cardClass} title={tips.ma}>
            <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider mb-1">{t.structure}</p>
            <p className={`font-bold text-sm ${maBull ? marketColors.upText : 'text-gray-400'}`}>
                {maBull ? t.bullish : t.neutral}
            </p>
          </div>
          <div className={cardClass} title={tips.vol}>
             <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider mb-1">{t.momentum}</p>
             <p className={`font-bold text-sm ${stock.volume > stock.vol5 ? marketColors.upText : 'text-gray-400'}`}>
                 {stock.volume > stock.vol5 ? t.highVol : t.lowVol}
             </p>
          </div>
          <div className={cardClass} title={tips.atr}>
             ...
             <p className="font-bold dark:text-white text-sm">{stock.atr.toFixed(1)}</p>
          </div>
          <div className={cardClass} title={tips.rsi}>
             ...
             <p className={`font-bold text-sm ${stock.rsi > 75 ? marketColors.downText : stock.rsi < 25 ? marketColors.upText : 'dark:text-white'}`}>{stock.rsi.toFixed(1)}</p>
          </div>
        </div>
      );
    } 
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className={cardClass} title={tips.ma240}>
             ...
             <p className={`font-bold text-sm ${stock.price > stock.ma240 ? marketColors.upText : marketColors.downText}`}>{stock.price > stock.ma240 ? t.bullish : 'Bear'}</p>
          </div>
          <div className={cardClass} title={tips.roe}>
            ...
            <p className={`font-bold text-sm ${marketColors.upText}`}>{stock.roe.toFixed(1)}%</p>
          </div>
          <div className={cardClass} title={tips.yield}>
             ...
             <p className={`font-bold text-sm ${marketColors.upText}`}>{stock.dividendYield.toFixed(2)}%</p>
          </div>
          <div className={cardClass} title={tips.inst}>
             <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider mb-1">{t.instHold}</p>
             <p className={`font-bold text-sm ${stock.institutionalOwnership > 40 ? marketColors.upText : 'dark:text-white'}`}>{stock.institutionalOwnership.toFixed(1)}%</p>
          </div>
        </div>
    );
  };
  ...
  const renderStockDetail = () => {
    if (stockLoading) return (
        <div className="flex flex-col items-center justify-center h-64 text-gray-400"><RefreshIcon className="w-8 h-8 animate-spin mb-4 opacity-50" /><p className="text-sm font-medium">{t.loading}</p></div>
    );
    ...
    return (
      <div className="space-y-6 animate-fade-in pb-20">
        {/* Main Price Card */}
        <div className="bg-white dark:bg-dark-surface p-6 rounded-3xl shadow-sm relative overflow-hidden border border-gray-100 dark:border-gray-800">
        ...
        <div className="bg-white dark:bg-dark-surface p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800">
             <div className="flex items-center justify-between mb-6">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isOpen ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
        ...
        <div className="fixed bottom-6 left-4 right-4 max-w-5xl mx-auto z-20 flex justify-center pointer-events-none">
            <button 
                ...
                className="pointer-events-auto bg-primary text-white px-8 py-3.5 rounded-full shadow-xl shadow-blue-500/20 hover:bg-blue-600 hover:scale-105 active:scale-95 transition-all font-semibold flex items-center gap-2 backdrop-blur-md bg-opacity-95 text-sm"
            >
                <CalculatorIcon />
  ...
  const renderStockList = (stocks: StockData[], title: string) => {
      if (stocks.length === 0) return (
          <div className="flex flex-col items-center justify-center py-12 text-gray-400">
             <p className="text-sm font-medium opacity-60">No stocks found.</p>
          </div>
      );
                        ...
                        >
                            <div className="flex justify-between items-start"> 
                                <div className="flex flex-col">
                                    <h3 className="font-bold text-lg dark:text-white flex items-center gap-2 tracking-tight">
                                        {stock.symbol}
                                    </h3>
                                ...
                                <div className="text-right">
                                     <div className="flex items-center justify-end gap-2">
                                        ...
                                        <div className={`px-2 py-1 rounded-lg text-xs font-bold ${badgeColor} text-white min-w-[56px] text-center`}>
      ...
      <div className="animate-fade-in pb-20">
         <div className="flex flex-col md:flex-row justify-between items-center mb-4 px-2 gap-3">
            ...
            <button 
               ...
               className="w-full md:w-auto flex items-center justify-center gap-1.5 text-xs font-bold text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800/80 px-4 py-2 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700 transition active:scale-95"
            >
               <SortIcon className="w-3 h-3" />
               <span>
                   {sortOption === 'SUITABILITY' ? t.sortStrategy : sortOption === 'CHANGE_DESC' ? t.sortChange : t.sortPrice}
               </span>
            </button>
         </div>
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark' : ''} font-sans selection:bg-primary selection:text-white`}>
      <header className="sticky top-0 z-30 bg-light-bg/90 dark:bg-dark-bg/90 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex-1 max-w-sm relative group">
             <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-4 py-1.5 rounded-lg bg-gray-200/50 dark:bg-gray-800/50 text-black dark:text-white focus:bg-white dark:focus:bg-gray-700 outline-none transition-all text-sm placeholder-gray-500 font-medium"
                />
             </div>
          ...
          <div className="flex items-center gap-3">
            <button 
                ...
                className="px-2.5 py-1 text-[11px] font-bold bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition"
            >
                {language === 'zh' ? 'EN' : '中'}
            </button>

            {user ? (
               <button onClick={handleLogout} className="w-8 h-8 rounded-full overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm">
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3 items-center">
         <div 
            ...
            className={`w-10 h-10 flex items-center justify-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-md text-gray-600 dark:text-gray-300 rounded-full shadow-lg border border-gray-100 dark:border-gray-700 cursor-pointer hover:scale-110 active:scale-90 transition-all ${isRefreshing ? 'animate-spin' : ''}`}
         >
         ...
         <div 
            ...
            className={`w-10 h-10 flex items-center justify-center ${isPaused ? 'bg-green-500' : 'bg-red-500'} text-white rounded-full shadow-lg cursor-pointer hover:scale-110 active:scale-90 transition-all`}
         >
